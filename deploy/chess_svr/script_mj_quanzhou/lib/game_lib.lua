...
function LibGameLogic:ProcessOPPlay(stPlayer, nCard)
    ...
    --下一轮时，去掉胡牌限制
    if GRoundInfo:GetYouSeat() == nChair then
        for i=1,PLAYER_NUMBER do
            if i ~= nChair then
                --给其它玩家加胡牌限制
                local stPlayeroTher =GGameState:GetPlayerByChair(i)   
                stPlayeroTher:SetOnlyHuType(0)
            end
        end
    end
    --出的牌是金牌
    if LibGoldCard:IsGoldCard(nCard) then
        ...
        if nGameStyle == GAME_STYLE_QUANZHOU then
            local envtest  = LibFanCounter:CollectEnv(stPlayer:GetChairID())
            --初始化，只用游金判断
            envtest.bDoubleYou =0
            envtest.bTribleYou =0
            ...
            if stPlayer:IsWin() == false and #stTingInfo>0 then
                ...
                for i=1,#stTingInfo do
                    local nGiveCard = stTingInfo[i].give 
                     --如果出的牌和听牌时预打出的牌都是金牌，并且fantype是游金，分为双游，三游2种
                    if LibGoldCard:IsGoldCard(nGiveCard) and stTingInfo[i].bIsYouJin then
                        if stPlayer:GetPlayYouStatus() ==0 then
                            stPlayer:SetPlayYouStatus(2)
                            GRoundInfo:SaveYouSeat(nChair)
                            CSMessage.NotifyPlayerYouStatus(nChair, 2)
                        elseif stPlayer:GetPlayYouStatus() ==2 then
                            stPlayer:SetPlayYouStatus(3)
                            GRoundInfo:SaveYouSeat(nChair)
                            CSMessage.NotifyPlayerYouStatus(nChair, 3)
                        end
                        for i=1,PLAYER_NUMBER do
                            if i ~= nChair then
                                --给其它玩家加胡牌限制
                                local stPlayerOne =GGameState:GetPlayerByChair(i)   
                                 --打金牌 1自摸 2杠上花
                                if stPlayer:GetPlayYouStatus() ==2 then
                                    stPlayerOne:SetOnlyHuType(1)
                                elseif  stPlayer:GetPlayYouStatus() ==3 then
                                    stPlayerOne:SetOnlyHuType(2)
                                end
                            end
                        end                      
                        break;
...
function LibGameLogic:ProcessOPQuadruplet(stPlayer, nCard)
    ...
    elseif GGameCfg.RoomSetting.nGameStyle == GAME_STYLE_QUANZHOU then 
        LibGameLogicQuanZhou:ProcessOPQuadruplet(nGangType,nChair,nTurn,nCard)
    end
    self:ClearAllBlock()
...
function LibGameLogic:ProcessOPWin(stPlayer, nCard)
    ...
    LibGameLogicQuanZhou:ProcessOPWin(stWinData)
...
function LibGameLogic:RewardThisGame()
    ...
    elseif nGameStyle == GAME_STYLE_QUANZHOU then
        LibGameLogicQuanZhou:RewardThisGame()
...
function LibGameLogic:ProcessOPSwitchBlock()
        ...
        if stRoundInfo:IsGunHu() then
            ...
            for i=1,PLAYER_NUMBER do
                ...
                if stBlockState:GetBlockRecordFlag() == ACTION_WIN then
                    -- 找到可以金雀的玩家
                    local bFind = LibGameLogicFuzhou:ChectFanCount(i, nRecordCard, 34)
                    if bFind then
                        stGoldChair[i] = i
                    end
                end
            end
            if #stGoldChair > 0 then
                for i=1,PLAYER_NUMBER do
                    nChair = LibTurnOrder:GetNextTurn( nChair )
                    if stGoldChair[nChair] then
                        stBlockState = self:GetPlayerBlockState(nChair)
                        local stPlayer = stGameState:GetPlayerByChair(nChair)
                        LOG_DEBUG("Gold bird nChair:%d !!!!!!!!!!!!!\n", nChair)
    ...
    for i=1,PLAYER_NUMBER do
        ...
        if stBlockState:GetBlockRecordFlag() == ACTION_WIN then
            local stPlayer = stGameState:GetPlayerByChair(nChair)
            LOG_DEBUG("--==GetBlockRecordFlag()-ACTION_WIN, WhoIsOnTurn:%d !!!!!!!!!!!!!\n", stRoundInfo:GetWhoIsOnTurn())
            self:ProcessOPWin(stPlayer, stBlockState:GetBlockaRecordCard())
            return bIsProcessed
        end
    end
