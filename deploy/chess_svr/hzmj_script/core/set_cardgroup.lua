...
-- 将杠降为碰
function SetCardGroup:Quadruplet2Triplet(nCard)
    for i=1, self.m_nCurrentLength do
        if self.m_setcards[i].ucFlag == ACTION_QUADRUPLET_REVEALED and self.m_setcards[i].card == nCard then
            self.m_setcards[i].ucFlag = ACTION_TRIPLET
            return true
        end
    end
    return false
