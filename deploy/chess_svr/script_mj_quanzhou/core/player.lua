...
function Player:InitPlayerGameInfo()
    ...
    self.m_bis_self_giveup = false

    self.m_nPlayerQiangGangHu =false

    self.m_nOnlyHuType = 0      --1自摸 2杠上花
    self.m_bRobEnd = false

    self.m_nYouStatus= 0        --0正常，1，游金，2双游，3双游
end

function Player:SetPlayYouStatus(nStatus)
    self.m_nYouStatus = nStatus
end

function Player:GetPlayYouStatus()
    return self.m_nYouStatus
end
...
function Player:GetIsUserSelfGiveup()
    return self.m_bis_self_giveup
end

function Player:SetIsUserSelfGiveup(bIsGiveUp)
    self.m_bis_self_giveup = bIsGiveUp
end
...
function Player:GetPlayerIsQiangGangHu() 
    return self.m_nPlayerQiangGangHu
end

function Player:SetPlayerIsQiangGangHu(isQiangGangHu)
    self.m_nPlayerQiangGangHu = isQiangGangHu
end
...
function Player:IsOwner()
    LOG_DEBUG("Player:IsOwner, myUidtype: %s, myUid: %d, ownerUid: %d", type(self.m_stUserInfo.user._uid), self.m_stUserInfo.user._uid, GGameCfg.uid)
    return self.m_stUserInfo.user._uid == GGameCfg.uid
end

--花牌
function Player:AddFlowerCard(nCard)
    table.insert(self.m_stFlowerCards, nCard)
end

function Player:GetFlowerCards()
    local t = {}
    t = Array.Clone(self.m_stFlowerCards)
    return t
end

function Player:GetFlowerNums()
    return #self.m_stFlowerCards
end
...
-- 打金牌 1自摸 2杠上花
function Player:SetOnlyHuType(nOnlyHuType)
    self.m_nOnlyHuType = nOnlyHuType
end

function Player:GetOnlyHuType()
    return self.m_nOnlyHuType
end

-- 抢金
function Player:SetRobEnd(bRobEnd)
    self.m_bRobEnd = bRobEnd
end

function Player:IsRobEnd()
    return self.m_bRobEnd
end
...
function Player:CheckCanTing()
    ...
    for j=1, nCardNums do
        if j % 10 ~= 0 then
            ...
            stPlayerCardArray[#stPlayerCardArray] = j
            if LibRuleWin:CanWin(stPlayerCardArray) then
                return true
            end
        end
    end
