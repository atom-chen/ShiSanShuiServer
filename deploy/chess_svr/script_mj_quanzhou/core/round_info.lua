...
function RoundInfo:ctor()  
    ...
    self.m_stLimit = false
    self.m_stYouSeat = 0
    self.m_nPengGangCard = 0  -- 碰杠牌
end
...
--是否还在游金双游三游限制之中
function RoundInfo:SetIsOnYouLimit(bIsLimit)
    self.m_stLimit = bIsLimit
end

function RoundInfo:GetIsOnYouLimit()
    return self.m_stLimit
end

function RoundInfo:SaveYouSeat(nSeat)
    self.m_stYouSeat = nSeat
end

function RoundInfo:GetYouSeat()
    return self.m_stYouSeat
end
