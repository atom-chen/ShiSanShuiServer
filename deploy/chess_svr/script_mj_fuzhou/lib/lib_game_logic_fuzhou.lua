...
function LibGameLogicFuzhou:ProcessOPWin(stWinData)
   ...
   -- LOG_DEBUG("LibGameLogicFuzhou:ProcessOPWin...nWinner:%d", nWinner)
    ...
    if GRoundInfo:GetHuWay() ==4 then
        stWinData[1].winType ="selfdraw"
    end
...
function LibGameLogicFuzhou:DoHuBalance(stWinData)
    ...
    for k=1, #self.m_stWinChairs do
        ...
        --CollectEnv(nChair, nTurn, nFlag, nLast)
    ...
    self.m_nHuInfo.nFanDetailInfo = {} --stFanInfo
    --三金倒得话，先判断是不是金龙金雀/ 清一色
	if bIsSanJinDao  and self.m_nHuInfo.nGoldDragon ~=1 and self.m_nHuInfo.nGoldBird ~=1 and self.m_nHuInfo.nHalfQYS ~=1 and self.m_nHuInfo.nQYS ~=1 then
       for i=1,#stFanInfo do
            --三金倒时不判断平胡/不计算闲金,删除闲金、平胡的番型,
            --三金倒可以胡天地胡
            --三金倒又不可以胡天地胡了！！！！！！！！！！！！！
            if stFanInfo[i].byFanType ==0 or stFanInfo[i].byFanType ==31  or stFanInfo[i].byFanType ==16 or stFanInfo[i].byFanType ==17 then
                self.m_nHuInfo.nFanNum = self.m_nHuInfo.nFanNum -stFanInfo[i].byFanNumber
               -- table.remove(stFanInfo,i)
                if self.m_nHuInfo.nXianJin ==1 then
                    self.m_nHuInfo.nXianJin =0
                end
            else
                self.m_nHuInfo.nFanDetailInfo[#self.m_nHuInfo.nFanDetailInfo+1] = stFanInfo[i]
            end
        end
	...
	if stWinOne.winner == stWinOne.winWho  or GRoundInfo:GetHuWay() ==4 then
		for i=1,PLAYER_NUMBER do
...
function LibGameLogicFuzhou:GetFanCount_Gun(nChair, nCard)
    ...
    LOG_DEBUG("GetFanCount_Gun...stFanCount:%s", vardump(stFanCount))
    --过滤掉三金倒 C++没有主动过滤掉了
    ...
    for i=1,#stFanCount do
        nFanNum = nFanNum + stFanCount[i].byFanNumber
        if stFanCount[i].byFanType == 34  or stFanCount[i].byFanType == 35 then
            bDragon =true
        end
        if stFanCount[i].byFanType == 31 then
            bxianjin =true
        end
        if stFanCount[i].byFanType == 34 then
            bBird =true
        end
        if stFanCount[i].byFanType == 35 then
            bLong =true
        end
        if stFanCount[i].byFanType == 36 then
            bHalfQYS =true
        end
        if stFanCount[i].byFanType == 37 then
            bQYS =true
        end
        if stFanCount[i].byFanType == 32 then
            bWuHuaWuGang =true
        end
    ...
    return nFanNum,bDragon,bBird,bLong,bHalfQYS,bQYS,bxianjin,bWuHuaWuGang
end

--点炮时的检查，检查是不是闲金（首先winflag设为自摸）
function LibGameLogicFuzhou:GetFanCount_Gun_check(nChair, nCard)
    local nTurn = LibTurnOrder:GetNextTurn(nChair) 
    local nFlag = WIN_SELFDRAW
    local nLast = nCard
    local env = LibFanCounter:CollectEnv(nChair, nTurn, nFlag, nLast)
    ...
    return nFanNum,bDragon,bXianJin
end
...
function LibGameLogicFuzhou:RewardThisGame()
    ...
    for i=1, PLAYER_NUMBER do
        ...
        if stPlayer:IsWin() then
            ...
            --手牌三金倒时算自摸
            if GRoundInfo:GetHuWay() ==4 and win_type[i] == "gunwin"then
                win_type[i] ="selfdraw" 
            end
