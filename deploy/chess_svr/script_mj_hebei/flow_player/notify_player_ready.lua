...
local function logic_notify_player_ready(stPlayer, event)
    ...
    -- 第一局才下发，否则只问
    local bNeedSend = stPlayer:IsNeedSend()
    if bNeedSend then
        -- 下发游戏配置
    ...
    -- 第一局才下发，否则只问
    if stPlayer:GetPlayerStatus() == PLAYER_STATUS_SIT and bNeedSend then
        -- 同步其他玩家的状态
    ...
    stPlayer:SetNeedSend(false)

    -- 通知玩家 举手操作
    if stPlayer:IsRobot() ==false then
        CSMessage.NotifyPlayerAskReady(stPlayer)
        return STEP_SUCCEED
    else
        LOG_DEBUG("Run LogicStep notify_player_ready,IsRobot nCurrJu: %d, (nPlayerStatus: %d == 1) ", GGameCfg.nCurrJu, stPlayer:GetPlayerStatus())
