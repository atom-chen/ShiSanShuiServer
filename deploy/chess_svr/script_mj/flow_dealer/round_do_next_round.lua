...
local function logic_round_do_next_round(dealer, msg)
    ...
    if stPlayer == nil then
        LOG_ERROR("stPlayer == nil thisTurn:%d", thisTurn);
        return STEP_FAILED
    end
...
-- 需不需要发一张牌给玩家 吃碰之后不摸牌
    ...
    if is_first_card then
        ...
        if stRoundInfo:IsNeedDraw() == true and nDealerCardLeft > 0 and nCardLocation > GGameCfg.nLeftCardNeedQuict then
        -- 发一张牌
            ...
            CSMessage.NotifyPlayerGiveCard(stPlayer,stCards,nDealerCardLeft,bLast)
            SSMessage.CallPlayerGiveCard(stPlayer)
        else
