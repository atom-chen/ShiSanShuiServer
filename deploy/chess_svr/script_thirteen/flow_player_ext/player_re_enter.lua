...
local function logic_player_re_enter(stPlayer, msg)
    ...
    elseif sCurrStage == "choose" then
        ...
        -- LOG_DEBUG("logic_player_re_enter..uid:%d, nChairid:%d, timerId:%d", stPlayer:GetUin(), nChairid, PLAYER_TIMER_ID_CHOOSE)
    ...
    if stPlayer:GetPlayerStatus() == PLAYER_STATUS_SIT then
        --通知准备+准备时钟
        --local nTimeout = FlowFramework.GetTimerLeftSecond(stPlayer:GetChairID(), PLAYER_TIMER_ID_READY) --有定时器
        --local nTimeout = GGameCfg.TimerSetting.readyTimeOut --无定时器

        --显示300s倒计时，时间结束后，玩家没有点击继续，直接结束整个牌局，弹大结算界面（仅修改十三水）
        local nTimeout = GDealer:GetLeftSeconds()
        CSMessage.ReNotifyPlayerAskReady(stPlayer, nTimeout)
    else
        --水庄 要把选择的倍数做通知
        if sCurrStage == "mult" then
            if GGameCfg.GameSetting.bSupportWaterBanker then
