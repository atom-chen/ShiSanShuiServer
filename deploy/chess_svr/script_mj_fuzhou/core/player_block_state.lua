...
--胡
function PlayerBlockState:SetCanWin(bCanWin,nCard, nFanNum,nBigType)
    if bCanWin == true then
        self.m_nCanWin = ACTION_WIN
        self.m_nCardWin = nCard
        self.m_nFanNum = nFanNum
        if nBigType~=0 then
            self.m_nCanWinBigCards =nBigType
        else
            self.m_nCanWinBigCards  = ACTION_WIN
        end
    else
        self.m_nCanWin = 0
        self.m_nCardWin = 0
...
function PlayerBlockState:GetCurrWinBigType()
    return self.m_nCanWinBigCards
end
