...
function PlayerCompareResult:CalculateTotallScore()
    local nTotall = 0
    --水庄不支持全垒打
    if GGameCfg.GameSetting.bSupportWaterBanker == false then
        local nAllShootChairID = GDealer:GetAllShootChairID()
        for _, v in pairs(self.m_stScoreResult) do
            local nFinalScore = v.nFinalScore
            if self.m_nAllShoot < 0 and v.toChairid == nAllShootChairID then
                nFinalScore = 2 * v.nFinalScore
            end
            nTotall = nTotall + nFinalScore
        end
        --玩家自己全垒打
        if self.m_nAllShoot > 0 then
            nTotall = nTotall * 2
        end
    else
        for _, v in pairs(self.m_stScoreResult) do
