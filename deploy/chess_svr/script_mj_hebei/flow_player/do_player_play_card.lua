...
local function logic_do_player_play_card(stPlayer, msg)
    ...
    if not GGameCfg.GameSetting.bSupportPlayCollect then 
        ...
        if nGiveStatus == GIVE_STATUS_COLLECT and nLastCard == card then
            LOG_DEBUG("logic_do_player_play_card...donot paly same card! nGiveStatus: %d, nLastCard: %d, card: %d", nGiveStatus, nLastCard, card)
