...
function Player:InitPlayerGameInfo()
    ...
    self.m_nWinType = -1   --添加胡牌类型（只区分自摸和点炮,自摸0，点炮1）
...
function Player:SetWinType(nFlag)
    self.m_nWinType = nFlag
end

function Player:GetWinType()
    return self.m_nWinType
end
...
function Player:AddMoney(nAdd)
    ...
    LOG_DEBUG("SetUserGameScore add :%d now:%d", nMoneyAdd, self.m_stUserInfo.score.coin)
