...
local stSetCard = {
    ...
    value = 0,  -- 碰谁的   -- 吃的哪一张 (0, 1, 2) card+0,+1,+2
} 
...
function SetCardGroup:AddSetCard(ucFlag, card, value)
    if self.m_nCurrentLength < 0 or self.m_nCurrentLength > 5 then
        return false
    end
...
function SetCardGroup:SetCurrentLength(nCurrentLength)
    if nCurrentLength <= 0 or nCurrentLength > 5 then
        return false
    end
...
function SetCardGroup:SetSetCard(nIndex, setCards)
    if nIndex <= 0 or nIndex > 5 then
        return false
    end
    local target = self.m_setcards[nIndex]
    target.ucFlag = setCards.ucFlag
    target.card = setCards.card
    target.value = setCards.value
    return true
end
