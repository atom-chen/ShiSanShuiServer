...
function LibSpCardLogic:GetSpecialType(cards)
    ...
    --8同
    elseif self:Check_Pt_Sp_Eight(tempCards) then
        LibNormalCardLogic:Sort(cards)
        return GStars_Special_Type.PT_SP_EIGHT
    ...
    --7同
    elseif self:Check_Pt_Sp_Seven(tempCards) then
        LibNormalCardLogic:Sort(cards)
        return GStars_Special_Type.PT_SP_SEVEN
    ...
    if nLaziCount == 0 then
        ...
        for _, v in pairs(flush) do
            if #v > 0 then
                ...
                if #v == 3 or #v == 5 then
                    local bStraight = LibNormalCardLogic:IsStraight(v)
                    -- LOG_DEBUG("Check_Pt_Sp_Three_Straight_Flush===========bStraight:%s", tostring(bStraight))
                    -- LOG_DEBUG("Check_Pt_Sp_Three_Straight_Flush===========v:%s", TableToString(v))
        ...
        --2色 3色
        local bthirdFind, thirdAllResult = libRecomand:Get_Pt_Straight_Flush_Laizi(normalCards, nLaziCount)
        if bthirdFind == false then
            return false
        end
        for _, thirdResult in ipairs(thirdAllResult) do
            for _, thi in ipairs(thirdResult) do
                ...
                ----2.中墩
                local bSecFind, secondAllResult = libRecomand:Get_Pt_Straight_Flush_Laizi(thirdLeftCards, thirdLeftLaizi)
                if bSecFind == false then
                    break
                end
                for _, secondResult in ipairs(secondAllResult) do
                    for _, sec in ipairs(secondResult) do
                        ...
                        --判断是否是同花顺
                        local bStraight = LibNormalCardLogic:IsStraight_Laizi(secondLeftCards, secondLeftLaizi)
                        local bFlush = LibNormalCardLogic:IsFlush(secondLeftCards)
                        if bStraight and bFlush then
                            return true
                        end
                    end
                end
...
--8同
function LibSpCardLogic:Check_Pt_Sp_Eight(cards)
    --assert(#cards == MAX_HAND_CARD_NUM)
    --1.把癞子牌和普通牌分离
    local normalCards = {}
    local laiziCards = {}
...
--7同
function LibSpCardLogic:Check_Pt_Sp_Seven(cards)
    --assert(#cards == MAX_HAND_CARD_NUM)
    --1.把癞子牌和普通牌分离
    local normalCards = {}
    local laiziCards = {}
...
function LibSpCardLogic:Check_Pt_Sp_Six_Pairs(cards)
    ...
    for k, v in ipairs(values) do
        if v > 0 then
            ...
            if n >=1 then
                if mf == 0 and nLaziCount > 0 then
                    n = n + 1
                    nLaziCount = nLaziCount - 1
                end
                count6 = count6 + n
            else
                --癞子凑
