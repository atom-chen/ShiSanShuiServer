...
function LibChangeCard:GetChangeCardResCard(nChairID)
    ...
    if self.m_nChangeCardType == EN_CHANGCARDTYPE_CLOCKWISE then
        -- 顺时针交换
        nNewIndex = (nChairID - 1+ PLAYER_NUMBER) % PLAYER_NUMBER
        if nNewIndex ==0 then
            nNewIndex = PLAYER_NUMBER
        end
    elseif  self.m_nChangeCardType == EN_CHANGCARDTYPE_ANTI_CLOCKWISE then
        -- 逆时针
        nNewIndex = (nChairID + 1 ) % PLAYER_NUMBER
        if nNewIndex ==0 then
            nNewIndex = PLAYER_NUMBER
        end
    elseif  self.m_nChangeCardType == EN_CHANGCARDTYPE_OPPOSITE then
        -- 对家交换
        if PLAYER_NUMBER ==4 then
            nNewIndex = (nChairID + 2) % PLAYER_NUMBER
            if nNewIndex ==0 then
                nNewIndex = PLAYER_NUMBER
            end
        else
    ...
    LOG_DEBUG("=================.%s     ==:%d\n", vardump(self.m_stState),nNewIndex)
