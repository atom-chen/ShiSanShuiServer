...
function slot.Init(nMinWin, nBaseBet)
    ...
    slot.m_stFanCounter = _GameModule.MJCounter.CreateFanCounterChengDu()

    if slot.m_stFanCounter == nil then
        LOG_ERROR("CreateFanCounterChengDu  Failed. ");
        return false
    end
...
function slot.CheckWin(arrPlayerCards,nlaizicount,laizicard,ngamestyle)
    return slot.m_stFanCounter:CheckWin(arrPlayerCards,nlaizicount,laizicard,ngamestyle)
end
function slot.InitForNext()
     slot.m_stFanCounter:InitForNext()
end
function slot.GetTingInfo()
    return slot.m_stFanCounter:GetTingInfo()
end
return slot
