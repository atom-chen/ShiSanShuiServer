...
function LibFanCounter:CollectEnv(nChair, nTurn, nFlag, nLast)
    ...
    elseif GGameCfg.RoomSetting.nGameStyle == GAME_STYLE_QUANZHOU then  
        ...
        env.flower= nFlowerCount
        env.byLaiziCards =nLaiziCards
        env.bDoubleYou =bDoubleYou
        env.bTribleYou =bTribleYou
        
        if GGameCfg.GameSetting.bSupportSanJinDao then
            env.bySanJinDao = 1
        else
            env.bySanJinDao = 0
        end
    ...
    LOG_DEBUG("=====COLLECT===#bDoubleYou============:%d",bDoubleYou)
    LOG_DEBUG("=====COLLECT===#bTribleYou============:%d",bTribleYou)
