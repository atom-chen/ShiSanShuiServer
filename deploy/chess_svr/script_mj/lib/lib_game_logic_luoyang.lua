...
function LibGameLogicLuoyang:ProcessOPWin()
    ...
    if nGangci > 0 then
        ...
        stWinType = "gangci"
        stWinList = { winner = nChair, winWho = nChair, cardWin = nCard, winType = stWinType }
    else
        local nCard = LibCi:GetCi()
        ...
        stWinList = { winner = nChair, winWho = lastTurnChair, cardWin = nCard, winType = stWinType }
    end
...
function LibGameLogicLuoyang:RewardThisGame()
    ...
    local function CalcHuScore()
        ...
        for i=1,PLAYER_NUMBER do
            local stPlayer = GGameState:GetPlayerByChair(i)
            if stPlayer and stPlayer:IsWin() == true then
                --1. 判断是否是杠次
    ...
    for i=1, PLAYER_NUMBER do
        ...
        if stPlayer then
            ...
            local rec = {
                ...
                win_type    = win_type[i],
                baoci       = baoci[i],
                revealed_gang_count = desc_arr[i].ming_gang_count,
                conceled_gang_count = desc_arr[i].an_gang_count
            }
