        ...
        while index <= #stChairTimers do
            ...
            if timeNow - stTimer.lastDoTime >= stTimer.interval then
                ...
                if stTimer.id ==-1 then
                    ...
                    -- LOG_DEBUG("chair:%d SendEvent.timeNow=%d\n", nChair,timeNow);

                    self:SendTimerEvent(nChair, stTimer.id, timeNow)
...
function TimerMng:SendTimerEvent(nChair, nTimerID, timeNow)
    local event = {
        _cmd = "timeout",
        _src = timeNow or 33333,
        _para = "",
        _st = "event"
    }
    local stUserObject = GGameState:GetPlayerByChair(nChair)
    if stUserObject == nil then
        -- LOG_DEBUG("NILLLLLLLLLLLLLLLLLLLLLLLLLL, chair=%d, nTimerID=%d,timeNow=%d\n", nChair, nTimerID, timeNow)
